...
def sample_edge_points(uv, n_per_edge=10):
    pts = []
    for s, e in EDGES:
        a = uv[s]
        b = uv[e]
...
def main():
    ...
    base = obj.get('refined_with_sam2_edges', obj.get('calibrated_from_sam2_masks', {}))
    yaw0 = float(base.get('yaw_deg', 0.0))
    sr0 = float(base.get('scale_radius', 1.0))
    sh0 = float(base.get('scale_height', 1.0))

    # Initial translation as local offsets around t0.
    tr_init = np.zeros(3, dtype=np.float64)

    starts = []
    for dyaw in (-18, -9, 0, 9, 18):
        for msr in (0.85, 1.0, 1.15):
            for msh in (0.85, 1.0, 1.15):
                starts.append(np.array([yaw0 + dyaw, sr0 * msr, sh0 * msh, tr_init[0], tr_init[1], tr_init[2]], dtype=np.float64))

    def clamp(p):
        p[1] = np.clip(p[1], 0.45, 2.0)
        p[2] = np.clip(p[2], 0.35, 1.8)
        p[3:] = np.clip(p[3:], -0.006, 0.006)
        return p
    ...
    def eval_cost(p):
        ...
        for r in rows:
            ...
            if uv is None:
                continue
            feat = mfeat[r['name']]
            bb_m = feat['bbox']; ctr_m = feat['center']; dist = feat['dist']; mask = feat['mask']

            edge_pts = sample_edge_points(uv, n_per_edge=10)
            h, w = dist.shape
            dsum = 0.0
            for ep in edge_pts:
                ex = int(round(ep[0])); ey = int(round(ep[1]))
                if ex < 0 or ex >= w or ey < 0 or ey >= h:
                    dsum += 10.0
                else:
                    dsum += float(dist[ey, ex])
            ...
            # Keep center keypoint on mask.
            cx = int(round(uv[0, 0])); cy = int(round(uv[0, 1]))
            center_out = 0.0
            if cx < 0 or cx >= mask.shape[1] or cy < 0 or cy >= mask.shape[0] or mask[cy, cx] == 0:
                center_out = 0.4

            total += 0.65 * edge_cost + 0.55 * bbox_cost + 1.55 * center_cost + center_out
            count += 1

        if count < max(12, len(rows) // 3):
            return None
        return total / count

    # Coordinate descent stages (coarse -> fine)
    stages = [
        np.array([6.0, 0.14, 0.14, 0.0018, 0.0018, 0.0018], dtype=np.float64),
        np.array([2.2, 0.06, 0.06, 0.0008, 0.0008, 0.0008], dtype=np.float64),
        np.array([0.8, 0.025, 0.025, 0.00030, 0.00030, 0.00030], dtype=np.float64),
    ]
    ...
    for s in starts:
        ...
        if best_global is None or c < best_global:
            best_global = c
            best_p_global = p.copy()

    if best_p_global is None:
        raise RuntimeError('Joint optimization did not find any valid solution')

    yaw, sr, sh, tx, ty, tz = best_p_global
    R = make_R(yaw)
    half = half0 * np.array([sr, sr, sh], dtype=np.float64)
    key = build_keypoints(half)
    tw = t0 + R @ np.array([tx, ty, tz], dtype=np.float64)
    ...
    print('best', obj['joint_refined_with_sam2'])
